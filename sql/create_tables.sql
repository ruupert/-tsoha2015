...
CREATE OR REPLACE FUNCTION trigger_create_initial_user_timestamp() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
   NEW.created_at = now();
...
CREATE OR REPLACE FUNCTION trigger_create_initial_timestamp() RETURNS trigger 
  LANGUAGE plpgsql
  AS $$
BEGIN
   NEW.created_at = now();
...
CREATE OR REPLACE FUNCTION trigger_update_modified_timestamp() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
   NEW.modified_at = now();
